 ...
 * 항상 성공하고, 구독 식별자는 {@code noop-{bandId}}, 구독기간 종료는 요청 시각 + premiumPeriodDays(1년).
 */
class NoOpPaymentGatewayTest {

    private final NoOpPaymentGateway gateway =
            new NoOpPaymentGateway(new PlanProperties(365, 30, "PREMIUM_YEARLY", "-", "Asia/Seoul"));

    private static final Instant NOW = Instant.parse("2026-03-01T00:00:00Z");

    @Test
    void subscribe_succeeds_with_ref_and_period_end() {
        SubscriptionResult result = gateway.subscribe(new SubscribeCommand(7L, 1L, "PREMIUM_YEARLY", NOW));

        assertThat(result.success()).isTrue();
        assertThat(result.subscriptionRef()).isEqualTo("noop-7");
        assertThat(result.currentPeriodEnd()).isEqualTo(NOW.plus(365, ChronoUnit.DAYS));
