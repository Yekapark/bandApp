...
## 5단계 — Cloudflare 주황 구름 ⚠️ **켰다가 껐다 (2026-09-06)**

**켜 두면 한국에서 요청 하나에 700ms~1s 가 걸린다.** 무료 요금제가 한국 트래픽을
서울이 아니라 LA 로 돌리기 때문이다(`colo=LAX`). 끄면 64~119ms.
자세한 측정과 다시 켤 때의 판단 근거는 [NEXT.md §0](progress/NEXT.md).

아래는 다시 켜게 될 때를 위해 남겨 둔다.
