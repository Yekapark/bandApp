...
> **상태: 완료. 슬라이스 0~4 전부 끝났다(2026-09-09).** 운영은 `gateway=google` 로 돌고 있고,
> 구매·해지·만료·환불(REVOKED) 네 가지를 실기기·실결제로 확인했다 — §5-4 실측표 참고.
> 남은 것은 스토어 심사 제출이며, 그건 Phase 12 범위 밖이다.
> 이 문서는 다른 사람/AI 가 검토할 수 있게 "무엇을 왜 어떻게" 를 자세히 적는다.
> 마지막 갱신: 2026-09-09 (환불 검증 완료).
...
반영되는 통합 테스트가 통과한다. → **충족.** 통합 테스트 통과 + 구매·해지·만료·환불 전부
실기기 실결제로 확인(아래 §5-4, §6).
...
| 환불·사용 권한 취소 후 즉시 FREE | RTDN type 12 처리 → `band_plans` `tier=FREE`·토큰 제거, 밴드 READY 미디어 4건 `expires_at` = 강등 시각(유예 0) | ✅ |

**환불 검증 로그·DB (밴드 4, 2026-09-09):**

```
14:27:50  RTDN 처리 완료 type=4  bandId=4  messageId=21752805484140213   ← 구매
14:34:07  RTDN 처리 완료 type=12 bandId=4  messageId=21789448676278592   ← 환불·권한취소
14:34:09  RTDN: 모르는 구매 토큰 (밴드 없음) type=13 — 무시
```

```
band_plans:         band_id=4  tier=FREE  store=NULL  purchase_token=NULL  expires_at=NULL
                    updated_at = 2026-09-09 14:34:07.958262+00
media_attachments:  READY 4건 전부 expires_at = 2026-09-09 14:34:07.958262+00
```

`type=12` 로그 시각과 두 테이블의 시각이 같은 순간이다 — 웹훅이 강등을 만든 게 시각으로 붙는다.
2초 뒤 온 `type=13`(만료)이 **밴드를 못 찾은 것**도 증거다: 강등이 `purchase_token` 을 실제로
떼어냈기 때문에 그 다음 알림이 붙을 밴드가 없어진 것이다.

> Play Console 에서 **환불만 하고 "사용 권한 취소" 를 체크하지 않으면 `type=12` 가 오지 않는다.**
> 처음 시도가 이래서 헛돌았다. 사용 권한 취소를 Play Developer API 로 직접 쏘는
> `deploy/play-revoke.sh` 를 만들어 해결했다 (`docs/TROUBLESHOOTING.md` 맨 위).

---

### 5-5. 슬라이스 0 진행 상황 (2026-09-09)

슬라이스 0의 스토어·서버 설정은 모두 끝났고, 슬라이스 4 실기기 e2e도 **전부 통과했다.**
...
마지막으로 [#83](https://github.com/Yekapark/bandApp/pull/83) 배포 뒤 환불·사용 권한 취소 →
`type=12` → 즉시 FREE 강등까지 확인해 **Phase 12 를 닫았다.** 앱 스토어 심사 제출만 남았다.

| PR | 내용 | CI | 결과 |
|---|---|---|---|
| [#83](https://github.com/Yekapark/bandApp/pull/83) | 클라 구매 토큰 전달 픽스 + RTDN 처리 성공 로그 | CI `34362333068` · Deploy `34362793828` | ✅ 배포 후 환불 검증 통과 |
...
4. ~~**슬라이스 4**: 활성 구독 환불·사용 권한 취소 → REVOKED 즉시 강등 확인.~~ 완료
5. **스토어 심사 제출** — Phase 12 범위 밖. `docs/BACKLOG.md` 배포 요건 참고.
6. 후속(선택): 웹훅 OIDC 켜기(§7 · TROUBLESHOOTING "아직 안 고침"), `obfuscatedAccountId` 대조.
