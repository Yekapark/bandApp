...
> **이어서 할 일은 [NEXT.md](NEXT.md) 에 있다** — 못 끝낸 것·우선순위·실행 명령.
>
> **새 세션에서 클라이언트 작업을 이어받을 때 이 파일부터 읽는다.**
> 이 문서는 "지금 어디까지 됐고, 어떻게 이어가는지"를 담는 살아있는 문서다.
> 작업을 진행하면 아래 "현재 상태"와 "다음 할 일"을 갱신한다.
> 마지막 갱신: **2026-09-05** (앱 이름·아이콘 확정, 알림 목록·정산 탭, 합주 영상 업로드 →
> [2026-09-05-brand-notifications-settlement-video.md](2026-09-05-brand-notifications-settlement-video.md))
...
- **지도 제공자를 카카오로 통일**(2026-09-04) — `flutter_naver_map` → `kakao_map_sdk`.
  백엔드 지오코딩도 네이버 NCP → 카카오 로컬로 교체해 프로젝트에서 네이버가 완전히 빠졌다.
  **지도 전용 키가 없다** — 로그인에 쓰는 `KAKAO_NATIVE_APP_KEY` 를 그대로 쓰고,
  `NAVER_MAP_CLIENT_ID` dart-define 은 사라졌다. 카카오 콘솔에 패키지명
  (**`com.yeka.bandule`** — 2026-09-06 에 `com.example.bandapp_client` 에서 바꿨다)
  + 키 해시 등록이 필요하며, 이는 카카오 로그인에도 같이 쓰인다.
- **합주실 등록 폼에 지도**(2026-09-04) — 주소 검색 후보가 지도에 핀으로 뜨고, 핀/목록에서 고른
  **그 좌표가 그대로 저장된다**(서버 지오코딩 생략). 직접 타이핑한 주소만 서버가 지오코딩한다.
  SDK 인증 실패 시 앱이 죽지 않고 지도 자리에 안내만 남는다.
  **카카오맵은 ARM 전용이라 x86_64 에뮬레이터에서는 지도가 안 뜬다**(앱은 정상 동작) —
  자세한 건 [2026-09-04-kakao-map-in-room-form.md](2026-09-04-kakao-map-in-room-form.md) §7-B.
- **앱 이름·아이콘 확정**(2026-09-05) — 밴듈 (Bandule). 워드마크는 Bebas Neue 가 라틴 전용이라
  영문(`AppConfig.appNameEn`), 시스템 표시명은 한글(`appName`). 아이콘은 `client/brand/` 의
  SVG 에서 `tools/render_icons.py` 로 생성한다.
- **알림 목록 `/notifications` 구현**(2026-09-05) — 홈 종 배지가 실제 안 읽은 수를 센다.
  읽음 여부는 서버에 없고 기기에 밴드별 "마지막 확인 시각"을 저장한다. 백엔드는 V11 로
  발송 이력에 문구를 함께 남긴다.
- **정산 탭 `/settlements` 구현**(2026-09-05) — "내가 아직 안 낸 돈"이 맨 위. 정산 생성·수정은
  기존대로 일정 상세에서 한다. 이제 탭 5개가 모두 실제 화면을 가진다(showSoon 헬퍼 삭제).
- **합주 영상 업로드**(2026-09-05) — 720p 압축 + 스트리밍 업로드 + 상한 200MB.
  스트리밍을 먼저 고쳐야 한다(그것 없이 상한만 올리면 앱이 죽는다).
- **게시글 영상 재생**(2026-09-04) — `video_player`. 목록에서는 재생 버튼 카드, 탭하면
  전체화면에서 재생(탭 play/pause, 스크러빙 진행바). chewie 는 안 붙였다.
- **글 쓰면서 사진·영상 고르기**(2026-09-04) — 첨부 영역이 수정 모드에서만 보여서 새 글에는
  사진 버튼이 아예 없었다. 이제 쓰는 중에 고르면 "등록 시 업로드"로 대기했다가, 등록 버튼에서
  글 생성 → 첨부 업로드를 이어서 한다. 일부 실패 시 실패분만 남긴다.
- **참석 여부가 화면을 벗어나면 되돌아가던 버그 수정**(2026-09-04) — 응답 뒤 로컬 상태만
  갱신하고 `reservationDetailProvider`(autoDispose 아님) 캐시를 안 비워서, 다시 들어오면
  옛 값이 그려졌다. 서버에는 정상 저장돼 있었다.
