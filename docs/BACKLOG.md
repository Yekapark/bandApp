...
- ~~**비밀번호 재설정("비밀번호 찾기") 플로우.**~~ **완료 (2026-09-06)** — Gmail SMTP로 6자리
  ...
  발송만 no-op(카카오·FCM과 같은 방식) — 앱 비밀번호 발급은 사람이 할 일(§1.x 참고, 구글 계정 >
  보안 > 2단계 인증 > 앱 비밀번호).
...
### 1.12 Phase 12 인앱결제 후속 (2026-09-09)

Phase 12 를 닫으면서(환불 REVOKED 실기기 검증 · 웹훅 OIDC 전용 전환 · RTDN 재전송 폭풍 수정,
PR #83·#84) 남겨둔 것들. 급하지 않지만 잊으면 다시 헤맨다.

- **[중] 옛 웹훅 공유 시크릿이 Nginx 접근 로그에 평문으로 남아 있다.** 푸시 URL 의
  `?token=<시크릿>` 이 쿼리스트링이라 접근 로그에 그대로 기록됐다. 2026-09-09 에 OIDC 전용으로
  바꾸면서 `.env.prod` 의 `PLAN_BILLING_WEBHOOK_SECRET` 을 비웠으니 **지금은 아무 데도 안 쓰인다.**
  다만 값 자체는 로그에 남아 있으므로, OIDC 를 되돌릴 일이 생기면 **그 값을 재사용하지 말고
  새로 만든다.** 로그 로테이션으로 언젠가 사라지지만 기한을 따로 관리하진 않는다.
  (되돌리는 절차는 `docs/TROUBLESHOOTING.md` 의 OIDC 항목.)

- **[하] 밴드에 안 붙은 grant 이벤트의 재전송이 애초에 불필요할 수 있다.** 지금은 1시간 제한만
  걸어 뒀다(`GRANT_RETRY_WINDOW`). 하지만 구매 토큰을 밴드에 붙이는 경로는 클라이언트
  `verify`(`StoreSubscriptionService.verifyGooglePurchase`) 하나뿐이고, `verify` 가 붙이는
  그 시점에 이미 스토어 조회로 PREMIUM 과 만료일이 확정된다. 그러면 나중에 재전송된 웹훅이
  밴드를 찾아도 **더 얹을 게 없다** — 즉 이 재시도는 통째로 지워도 되는 코드로 보인다.
  결제 경로의 안전망이라 분석만으로 지우지 않고 남겨 뒀다. 지우려면 먼저 확인할 것:
  토큰을 붙이는 다른 경로가 정말 없는지(`grantPremium` 호출자 전부), 그리고 `verify` 가
  `PLAN_ALREADY_PREMIUM` 경합으로 빠지는 분기에서도 토큰이 붙는지.

- **[하] `processed_store_events` 에 정리 배치가 없다.** RTDN 웹훅 중복 처리를 막는 멱등 테이블인데
  행을 지우는 코드가 없어 영원히 쌓인다. 구매 토큰이 함께 들어 있어 개인정보처리방침에도
  "중복 반영을 막기 위해 보관" 이라고만 적어 뒀다. 결제가 늘면 오래된 행(예: 90일 초과)을 지우는
  배치를 붙이고 방침의 보관 기간도 그 값으로 바꾼다.

- **[하] `obfuscatedAccountId` 대조 없음.** 지금은 "한 토큰 = 한 밴드" 만으로 재사용을 막는다.
  Play Billing 구매 시 `obfuscatedAccountId` 에 밴드 id 를 실어 보내고 서버가 대조하면 더 강하다.
  (`docs/progress/phase-12-iap.md` §7 에도 있음)

- **[하] acknowledge 실패 재시도 잡 없음.** 실패 시 로그만 남는다. 3일 안에 acknowledge 가 안 되면
  Play 가 자동환불하고 REVOKED 웹훅으로 자기수정되지만, 그 사이 사용자는 "PREMIUM 인데 곧 환불"
  상태다. (같은 §7)
